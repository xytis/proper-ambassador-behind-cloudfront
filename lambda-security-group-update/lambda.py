...
def lambda_handler(event, context):
    ...
    group_ids = update_security_groups(cf_ranges)

    # Notify next lambda
    notify_downstream_sns(group_ids)
...
def update_security_groups(new_ranges):
    ...
    # Extract group ids
    group_ids = [gr['GroupId'] for gr in rangeToUpdate['SecurityGroups']]
...
def notify_downstream_sns(group_ids):
    if SNS_TOPIC_ARN=="":
        return
